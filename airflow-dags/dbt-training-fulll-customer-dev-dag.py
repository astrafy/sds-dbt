...
# Local imports
from dags.utils.k8s_utils import gke_bash
...
def construct_dbt_snapshot_command(ENV):
    snapshot_command = f"""
        dbt snapshot --target={ENV} --profiles-dir=/app; exit $?;
    """
...
with DAG(
    ...
    with TaskGroup(group_id="Distribution") as Distribution:
        message = {"data": b"Customer updated" }
    ...
    def get_dbt_mode_parameter(**kwargs):
        ...
        run_stg = construct_dbt_run_command("stg_customer", ENV, mode)
        run_int = construct_dbt_run_command("int_customer", ENV,mode)
        run_dm = construct_dbt_run_command("mart_customer", ENV,mode)
        run_snapshot = construct_dbt_snapshot_command(ENV)
        return run_stg,run_int,run_dm,run_snapshot
